...
model_name = "heavy_model_26_8_1.5", # YOUR MODEL YOU USED FORTRAINING
...
for query in tqdm(natural_language_queries, desc="Generating MongoDB Queries"):
    ...
    For queries involving confidence (e.g., "score greater than 70 percent"), use `score` with operators like `$gt`, `$lt` (e.g., `"$gt": 0.7`).
        Today is 2025-08-22T19:52:42Z use only when the query explicitly mentions to use the current date and time like using words today, last week. Never include or infer the date unless strictly mentioned in the query.
...
output_csv_file = "query_test_results_27_8_1.5.csv"
