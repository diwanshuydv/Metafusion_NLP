...
def has_timestamp_key(query_obj: dict) -> bool:
    query_dict = query_obj.get("query", {})
    return "response.event.c_timestamp" in query_dict
def convert_timestamps_to_ist(data: dict) -> dict:

    processed_data = copy.deepcopy(data)
    ist_zone = ZoneInfo("Asia/Kolkata")
    def recursive_converter(item):
        if isinstance(item, dict):
            for key, value in item.items():
                item[key] = recursive_converter(value)
        elif isinstance(item, list):

            for i, value in enumerate(item):
                item[i] = recursive_converter(value)
        elif isinstance(item, str) and item.endswith("Z"):

            try:
                naive_dt_str = item.rstrip("Z")
                naive_dt = datetime.datetime.fromisoformat(naive_dt_str)

                aware_dt = naive_dt.replace(tzinfo=ist_zone)
                return aware_dt.isoformat()
            except ValueError:
                return item
        return item
    return recursive_converter(processed_data)
...
prefix = " The current date and time is " + str(formatted_dt)
...
q5= prefix + " Find all events of vehicle with the exact number plate being GJ10SS1010"
...
q10= prefix + " Find vehicle with license plate starting with GJ"
...
examples_data = [
    ...
    (q41, a41), (q42, a42), (q43, a43), (q44, a44), (q45, a45), (q46, a46), (q47, a47), (q48, a48),  (q49, a49),(q50,a50) ,(q51,a51)
]
...
#  f.write(full_initial_prompt_content)
...
async def query(request: QueryRequest):
    ...
    if not text.startswith("desc"):
        ...
        try:
            ...
            parsed_json_obj=            convert_timestamps_to_ist( parsed_json_obj )
