...
def convert_mongo_to_pymongo(mongo_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # If any error occurs, keep the original query for reconstruction
        components['original_filter'] = original_query
    ...
    if limit_match:
        ...
        original_limit_match = re.search(r'\.limit\(\s*(\d+)\s*\)', original_query, re.DOTALL)
...
def reconstruct_mongo_query(pymongo_dict: Dict[str, Any]) -> str:
    ...
    if pymongo_dict.get('operation') == 'countDocuments':
        ...
        if 'original_query' in pymongo_dict:
            original = pymongo_dict['original_query']
            if not original.strip().endswith(')'):
                # Don't add a closing parenthesis if the original didn't have one
                pass
            else:
                query += ')'
        else:
    ...
    # If we had a parse error or complex original filter, use original query for find part
    if 'original_filter' in pymongo_dict:
        # Extract from the original query
        original_query = pymongo_dict.get('original_query', '')
        # Use a more robust pattern to extract the full find(...) including nested braces
        find_pattern = r'(db\.event\.find\((?:[^()]|\([^()]*\))*\))'
        find_match = re.search(find_pattern, original_query, re.DOTALL)
        if find_match:
            base_query = find_match.group(1)
    ...
    # If we have the original query with math expressions, we can extract the find part
    if 'original_query' in pymongo_dict and 'math_expressions' in pymongo_dict:
        original_query = pymongo_dict['original_query']
        math_expressions = pymongo_dict['math_expressions']
        
        # Extract the find() part from the original query to preserve math expressions
        find_pattern = r'\.find\(\s*((?:{.*?}|.*?))\s*(?:,\s*({.*?})\s*)?\)'
        find_match = re.search(find_pattern, original_query, re.DOTALL)
        
        if find_match:
            query = "db.event.find(" + find_match.group(0)[6:]
    ...
    # If we don't have original query info, fall back to standard reconstruction
    ...
    if pymongo_dict['projection']:
        ...
        # Check if the original query already had a closing parenthesis
        # Skip adding if it's already there
        if 'original_query' in pymongo_dict:
            original = pymongo_dict['original_query']
            # If the original query is missing a closing parenthesis, don't add one
            if not original.strip().endswith(')'):
                query = query.rstrip(')')
            else:
                query += ')'
